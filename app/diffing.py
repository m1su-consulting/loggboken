...
def compute_environment_diff(
    left_installations: list[tuple[str, str, str]],
    right_installations: list[tuple[str, str, str]],
) -> list[DiffItem]:
    """Diffs two sets of (artifact_name, version, environment_name) active
    installations.
    ...
    specific environment/namespace it came from, so the UI can show exactly
    where an artifact lives rather than just the aggregate query name.
    """
    left_by_artifact: dict[str, list[EnvironmentVersion]] = defaultdict(list)
    for name, version, environment_name in left_installations:
        left_by_artifact[name].append(
            {"environment_name": environment_name, "version": version}
        )

    right_by_artifact: dict[str, list[EnvironmentVersion]] = defaultdict(list)
    for name, version, environment_name in right_installations:
